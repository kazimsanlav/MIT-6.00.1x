...
def isValidWord(word, hand, wordList):
    ...
    if word not in wordList:
        return False
    wordlist = getFrequencyDict(word)
    try:
        for ch in wordlist.keys():
            if not int(wordlist[ch]) <= int(hand[ch]):
                return False
        return True    
    except:
        return False
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print('"'+inp+'" earned '+ str(getWordScore(inp, n)) +' points.', end="   ")
                total_score += getWordScore(inp, n)
                print('Total: '+ str(total_score)+' points', end="\n")
                # Update the hand 
